...
# 3. install typescript, express, dotenv, pg, and sequelie
npm install express pg typescript ts-node dotenv sequelize sequelize-typescript pg-hstore
npm install --save-dev @types/node @types/express @types/pg @types/sequelize
...
# 6. Create a .env file for environment variables (db credentials)
cat > .env.example << EOL
...
PG_DB_NAME_TEST=db_postgres_test  # for test environment

SERVER_PORT=3000
EOL

# 7. Create src folder and sub folders
mkdir src
mkdir src/config
mkdir src/database
...
# 8. Create a database configuration file using modules:
...
# 8b. Create a database configuration file using common (for sequelize-cli and perhaps others):
cat > src/config/env-common.js << 'EOL'
...
module.exports = {
  ...
  test: {
    username: process.env.PG_DB_USER,
    password: process.env.PG_DB_PASSWORD,
    database: process.env.PG_DB_NAME_TEST,
    host: process.env.PG_DB_HOST,
    port: process.env.PG_DB_PORT,
    dialect: 'postgres'
  },
...
module.exports = {
  'config': path.resolve('src/config', 'env-common.js'),
...
# 10. Add build and startup scripts to package.json
cat > package.json << 'EOL'
{
  "name": "express-typescript-postgres",
  "version": "1.0.0",
  "description": "Backend API with Express, TypeScript and PostgreSQL",
  "main": "dist/server.js",
  "scripts": {
    "build": "tsc",
...
npm install cors express pg dotenv typescript ts-node sequelize sequelize-typescript pg-hstore

# Install dev dependencies
npm install --save-dev @types/cors @types/node @types/express @types/pg @types/sequelize nodemon typescript ts-jest @types/jest jest sequelize-cli
...
# In your setup, port 5000 is being used for your Express server, while port 5432 is the default PostgreSQL database port. Let's break this down:

# 1. Express Server (port 5000): [1]

        # This is your web server that handles HTTP requests

        # Defined in your server.ts with process.env.SERVER_PORT || 5000

        # When you visit http://localhost:5000, you're connecting to this Express server
...
# So when you make a request to http://localhost:5000:

        # 1. Express receives the request on port 5000
...
# This is a typical setup where:
        ...
        # Web/API server (Express) uses one port (5000)
...
# At localhost:5000/users should display the test user created by the script
...
# Then, when you GET http://localhost:5000/users, you should see the created user:
